...
def update_book(
    ...
    *,
    ...
    book = books_repo.fetch_by_id(session, bid)
    if book is None:
    ...
    # Empty mask means “replace all scalars from the request” (REST + backward-compatible gRPC).
    if not paths:
        t, a = _validate_title_author(title, author)
        isbn_clean = _require_non_empty(isbn, "isbn")
        year: Optional[int] = publication_year if publication_year and publication_year > 0 else None
        ...
        book.updated_at = timeutil.utc_now()
        if new_total is not None:
            book.total_copies = new_total
            book.available_copies = new_total - on_loan
    else:
        # Partial update: only listed proto paths change (gRPC review: FieldMask support).
        on_loan = book.total_copies - book.available_copies
        if "title" in paths:
            book.title = _require_non_empty(title, "title")
            if len(book.title) > _MAX_TITLE:
                raise ValidationError("title is too long", code="validation")
        if "author" in paths:
            book.author = _require_non_empty(author, "author")
        if "isbn" in paths:
            book.isbn = _require_non_empty(isbn, "isbn")
            if len(book.isbn) > 32:
                raise ValidationError("isbn is too long", code="validation")
        if "publication_year" in paths:
            year = publication_year if publication_year and publication_year > 0 else None
            if year is not None and (year < 1000 or year > 3000):
                raise ValidationError("publication_year is out of range", code="validation")
            book.publication_year = year
        if "total_copies" in paths:
            new_total = total_copies
            if new_total is None:
                raise ValidationError("total_copies is required when masked", code="validation")
            if new_total < 1:
                raise ValidationError("total_copies must be at least 1", code="validation")
